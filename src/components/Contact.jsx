...
export default function Contact() {
  ...
  return <section id="contact" className="section contact-section" aria-labelledby="contact-title"><div className="container contact-layout">
    <div><p className="eyebrow">{t("contact.eyebrow")}</p><h2 id="contact-title">{t("contact.title")}</h2><p className="contact-lead">{t("contact.subtitle")}</p><div className="contact-prompt"><span>{t("contact.promptLabel")}</span><p>{t("contact.prompt")}</p></div></div>
    <form onSubmit={handleSubmit} className="contact-form">
      ...
      <div className="form-actions"><button className="btn" type="submit" disabled={sending}>{sending ? t("contact.sending") : t("contact.cta")}</button>{msg && <span role="status" aria-live="polite" className={status}>{msg}</span>}</div>
    </form>
  </div></section>;
}
