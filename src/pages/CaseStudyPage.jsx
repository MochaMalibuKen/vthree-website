...
export default function CaseStudyPage() {
  ...
  return <main>
    ...
    <section className="section case-detail-page"><div className="container case-content">
      ...
      <div><h2>{labels.capabilities}</h2><div>{study.capabilities.map(item => <span className="tag" key={item}>{item}</span>)}</div><h2>{labels.future}</h2><p>{study.future}</p><a className="text-link" href={study.link} target="_blank" rel="noreferrer">{t("cases.cta")} <span aria-hidden="true">↗</span></a></div>
